...
class EtClient extends EtBaseClass {
	...
	public function __construct($username, $password, $serverinstance = 's4') {
			...
			$this->wsdl = "https://webservice.exacttarget.com/etframework.wsdl";
	...
	public function update($class, $activeClass, $updateType = "UPDATEADD")
	{
		...
		// throw exception with request and results
	...
	public function soapCall($object)
	{
			...
			$classNamespaceArray = explode('\\', $classNamespace);
			$className = end($classNamespaceArray);
			$classType = substr($className, 2);
